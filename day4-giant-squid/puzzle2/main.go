...
func checkifLastBingoCard(indexes []int, numberOfBingoBoards int) bool {
	var numberOfBingoCards int
	for i := 0; i < numberOfBingoBoards; i++ {
		for _, indexNumber := range indexes {
			if indexNumber == i {
				numberOfBingoCards += 1
				break
			}
		}
	}
	if numberOfBingoCards == numberOfBingoBoards {
		return true
	}
	return false
}
				...
				for bingoCardRowIndex, bingoCardNumber := range bingoCardRow {
					if bingoCardNumber.number == bingoNumber {
						...
						if checkBingoCard(bingoBoard) {
							bingoBoardIndexes = append(bingoBoardIndexes, bingoBoardIndex)
							if checkifLastBingoCard(bingoBoardIndexes, len(bingoBoards)) {
